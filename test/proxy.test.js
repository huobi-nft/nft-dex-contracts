...
describe("Test  proxy", async function () {
    ...
    describe("Test receiveApproval", async function () {
        beforeEach("Deploy mockContracts", async function () {
            ...
            this.msgVal = ethers.utils.parseEther("66");
            this.extra = ethers.utils.defaultAbiCoder.encode(
                ["string"],
                ["Test Extra"]
            );
            this.hexExtra = ethers.utils.arrayify(this.extra);
        });


        it("Expect revert , because the ecr20 token is not allowed", async function () {
            await expect(
                this.proxy.receiveApproval(
                    ...
                    this.usdt.address,
                    this.hexExtra)
            ).to.revertedWith("This ERC20 token is not supported");
        })

        it("Expect revert , because the user did not approve", async function () {
            await this.manager.connect(this.dao).setReceiveAllowed(this.usdt.address, true);
            await expect(
                this.proxy.receiveApproval(
                    this.usdtOwner.address,
                    this.msgVal,
